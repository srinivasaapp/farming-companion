...
export default function MarketPage() {
    const { t } = useLanguage();
    const [mainTab, setMainTab] = useState<'expert' | 'user'>('expert');
    ...
    return (
        <div className={styles.container}>
            <TopTabNav
                ...
                onExpertAdd={() => alert("Update Market Prices")}
                onUserAdd={handleCreate}
            />

            {/* EXPERT VIEW: Market Prices */}
            {mainTab === 'expert' && (
                <div className="p-4">
                    <h2 className="text-xl font-bold mb-4 text-green-900">{t('market_prices_title') || 'Mandi Prices'}</h2>
                    <div className="space-y-3">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="flex justify-between items-center p-4 bg-white rounded-xl shadow-sm border border-gray-100">
                                <div>
                                    <div className="font-bold text-gray-800">Tomato (Hybrid)</div>
                                    <div className="text-sm text-gray-500">Guntur Market</div>
                                </div>
